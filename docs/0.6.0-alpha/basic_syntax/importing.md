...
## Public Functions and Variables {#pub}

To declare a function or variable as public we can use a `pub` keyword. Let's keep in mind that `pub` keyword has to be used before the `fun`, `let`, or `const` keyword that declares our function/variable:
...
// Requires #[allow_public_mutable] flag
...
It's possible to import functions and variables individually.

```ab
import { foo, bar, MAX_RETRIES } from "./my-file.ab"
