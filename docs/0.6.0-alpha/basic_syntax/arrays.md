...
Note: Accessing an out-of-bounds index (e.g. `groceries[10]`) will now raise a runtime error, preventing silent failures.
...
You can easily destructure an array into separate variables, either when declaring new variables or reassigning existing ones:
```ab
...
[fruit1, fruit2, berry] = ["kiwi", "mango", "grape"]
```
